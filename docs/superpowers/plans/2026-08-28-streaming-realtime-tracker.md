...
- **`integrate_image` change:** `crop_size=None` -> `int(round(1.6 * person_height))`. 1.6 gives the person plus ~30% margin each side for limb extent and box drift. Keep the current default working when a number is passed.
- **Integrated crop:** call `integrate(result.window_frames, result.winner, crop_size=None, person_height=..., method='median')`. Median is the occlusion-robust fusion and is the entire point; `mask_background=False` keeps natural image statistics for the detector.
- **HOG:**
...
```
  ...
  Build the detector **once** at module import or in `main`, never per frame. `weights` are SVM decision values; draw each as e.g. `f"{w:.2f}"`. HOG wants the person roughly 64x128 or larger inside the window, so if the crop is small, upscale it 2x before detecting and scale the boxes back - state that in a comment.
