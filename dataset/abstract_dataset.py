...
DatasetEntry = namedtuple('DatasetEntry', 'orig_file gauss_file circle_file aniso_file bbs')
...
class AbstractDataSet(ABC, Dataset):
    ...
    def __init__(self, root_dir: str, seq_length: int, nth_frame: int, exclude_roots: List[str],
        ...
        bbs_per_root, hms_per_root, circle_per_root, aniso_per_root, imgs_per_root = \
            self.__search_files(root_dir, exclude_roots)
        self.entries, self.idx_mapping = self._construct_ds_entries(bbs_per_root, hms_per_root, circle_per_root,
                                                                    aniso_per_root, imgs_per_root)
    ...
    def __search_files(root_dir: str, exclude_roots: List[str]):
        ...
        aniso_per_root = [sorted(search_files(root, "_aniso", True), key=lambda x: x[1]) for root in roots]
        ...
        return bbs_per_root, gauss_per_root, circle_per_root, aniso_per_root, imgs_per_root
    ...
    def __getitem__(self, idx) -> Dict[str, torch.Tensor]:
        ...
        aniso = self.entries[idx].aniso_file
        ...
        # each heatmap channel independently falls back to zero if its file is missing (e.g.
        # aniso is absent for a frame with a degenerate/missing mask even though gauss/circle
        # exist) rather than zeroing the whole stack, so an all-or-nothing check on one channel
        # never corrupts the others
        zero_channel = np.zeros(img.shape, dtype=np.uint8)
        hm = np.stack([
            cv2.imread(gauss.path, 0) if gauss is not None else zero_channel,
            cv2.imread(circle.path, 0) if circle is not None else zero_channel,
            cv2.imread(aniso.path, 0) if aniso is not None else zero_channel,
        ], axis=2)
    ...
    @abstractmethod
    def _construct_ds_entries(self, bbs_per_root, hm_files_per_root, circle_per_root, aniso_per_root,
